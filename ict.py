...
class IctParams:
    ...
    def label(self) -> str:
        ...
        return (f"piv{self.pivot_len} arm{self.arm_bars} run{self.min_run_len} "
                f"rr{self.rr_mult:g} {mode}{' +ifvg' if self.use_ifvg else ''}")
...
def compute_signals(m: Market, p: IctParams):
    """
    Returns (side, sl, kind) arrays. side: +1 buy, -1 sell, 0 none.

    Everything here is decided on the CLOSE of bar i using only bars <= i.
    The target is deliberately NOT computed here: it depends on the real fill
    price, which is the next bar's open, so `simulate` derives it. That also
    makes these arrays independent of rr_mult, which the sweep caches on.
    """
    n = len(m.c)
...
def simulate(m: Market, p: IctParams, sig=None,
    ...
    """
    side, sl_a, kind_a = compute_signals(m, p) if sig is None else sig
    ...
    while i < end - 1:
        ...
        while j < end:
            ...
            if hit_sl or hit_tp:
                ...
                trades.append(Trade(is_buy, entry, sl, tp,
...
def stats(trades: list[Trade]) -> dict:
    if not trades:
        return {"n": 0, "net": 0.0, "r": 0.0, "avg_r": 0.0, "wr": 0.0, "pf": 0.0, "dd": 0.0}
...
def report(m: Market, p: IctParams, trades: list[Trade]) -> None:
    ...
    if getattr(simulate, "skipped", 0):
        print(f"\n({simulate.skipped} signals skipped — next open gapped through the stop)")
...
def run_sweep(m: Market, base: IctParams) -> None:
    ...
    rows = []
    cache: dict = {}
    for idx, (pl, ab, mr, rr, mode) in enumerate(combos, 1):
        if idx % 20 == 0:
        ...
        # signals do not depend on rr_mult, so one pass serves every target
        key = (pl, ab, mr, mode)
        if key not in cache:
    ...
    print("\n" + "=" * 78)
    print("TOP 12 BY TRAIN — the TEST column is the one that matters")
    print("=" * 78)
    print(f"{'settings':<40}{'train R':>9}{'train n':>9}{'test R':>9}{'test n':>8}")
    print("-" * 78)
    for p, tr, te in rows[:12]:
        print(f"{p.label():<40}{tr['avg_r']:>+9.2f}{tr['n']:>9}{te['avg_r']:>+9.2f}{te['n']:>8}")

    robust = [r for r in rows if r[2]["avg_r"] >= 0.10 and r[1]["avg_r"] > 0]
    robust.sort(key=lambda r: r[2]["avg_r"], reverse=True)
    print("\n" + "=" * 78)
    print("HELD UP OUT-OF-SAMPLE (test avg >= +0.10R and profitable on both)")
    print("=" * 78)
    if not robust:
        print("None.")
